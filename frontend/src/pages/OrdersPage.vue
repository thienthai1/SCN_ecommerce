<template>
  <div class="min-h-screen bg-stone-50 pb-20">
    <!-- Header -->
    <header
      class="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-gray-200"
    >
    ...
    <main v-else class="px-4 py-4">
      ...
      <!-- Filter Tabs -->
      <div
        class="flex flex-nowrap gap-2 mb-4 overflow-x-auto scrollbar-hide pb-1"
      >
        <button
      ...
      >
        <div
          class="w-16 h-16 rounded-full bg-stone-200 flex items-center justify-center mb-4"
        >
        ...
        >
          ...
          <div class="space-y-2 mb-3">
            ...
            >
              <!-- Item Image -->
              <div
                class="w-14 h-14 rounded-lg overflow-hidden bg-stone-200 flex-shrink-0"
              >
                <img
              ...
              <div class="flex-1 min-w-0">
                ...
                <p v-if="item.options" class="text-xs text-gray-500 mt-0.5">
                  <span
                    class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-gray-100 rounded text-gray-600"
                  >
          ...
          <div class="pt-3 border-t border-gray-100 space-y-1.5">
            <!-- Shipping Address -->
            <div
              v-if="order.shipped_address"
              class="flex items-start gap-2 text-xs pb-2 mb-2 border-b border-gray-100"
            >
              <q-icon
                name="location_on"
                size="14px"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-gray-500 mt-0.5 line-clamp-2">
            ...
            <div class="flex items-center justify-between text-xs">
              ...
              <span class="text-gray-600">{{
                formatPrice(
                  order.total_price - (order.shipping_price || 0) || 0,
                )
              }}</span>
            </div>
            <!-- Shipping Info -->
            ...
            <!-- Action Buttons -->
            <div
              class="mt-3 pt-3 border-t border-gray-100 flex justify-end gap-2"
            >
              <button
                v-if="
                  order.status === 'pending' &&
                  order.payment_status !== 'paid' &&
                  (!order.slipped_images || order.slipped_images.length === 0)
                "
              ...
              >
                <q-spinner
                  v-if="openingStripeOrderId === order.id"
                  color="white"
                  size="16px"
                />
                <span>{{
                  openingStripeOrderId === order.id
                    ? "Opening Stripe..."
                    : "Pay Now"
                }}</span>
              </button>
              ...
              <span
                v-if="
                  order.status === 'pending' &&
                  order.payment_status !== 'paid' &&
                  order.slipped_images &&
                  order.slipped_images.length > 0
                "
      ...
      <q-card class="payment-panel w-full max-w-md rounded-2xl">
        <q-card-section class="payment-panel-header text-center">
          ...
          <p class="text-sm text-gray-500 mt-1">
            Scan the bank QR code and upload your payment slip
          </p>
        </q-card-section>
        ...
        <q-card-section class="pt-0">
          ...
          <div
            v-if="selectedOrder"
            class="bg-emerald-50 border border-emerald-200 rounded-xl p-3 mb-4"
          >
          ...
          <div
            class="bg-white border-2 border-dashed border-gray-200 rounded-xl p-4 mb-4"
          >
            <div
              class="aspect-square max-w-xs mx-auto bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden"
            >
              <img
                v-if="paymentGateway?.imageUrl"
                :src="paymentGateway.imageUrl"
                alt="Payment QR"
            ...
            <div class="text-center mt-4">
              <p
                v-if="paymentGateway?.bankName"
                class="text-sm font-medium text-gray-700"
              >
                Bank: {{ paymentGateway.bankName }}
              </p>
              <p
                v-if="paymentGateway?.accountNumber"
                class="text-sm text-gray-600"
              >
                Account number: {{ paymentGateway.accountNumber }}
              </p>
              <p
                v-if="paymentGateway?.accountName"
                class="text-sm text-gray-600"
              >
                Account name: {{ paymentGateway.accountName }}
              </p>
            </div>
          </div>
