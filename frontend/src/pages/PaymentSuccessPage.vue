<template>
  <div class="min-h-screen bg-stone-50 flex items-center justify-center px-4">
    <div class="w-full max-w-md bg-white rounded-2xl p-6 shadow-sm text-center">
      <div :class="['w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4', isPaid ? 'bg-emerald-100' : 'bg-amber-100']">
        <q-icon :name="isPaid ? 'check_circle' : 'hourglass_top'" size="34px" :class="isPaid ? 'text-emerald-600' : 'text-amber-600'" />
      </div>
      <h1 class="text-xl font-bold text-gray-800">{{ isPaid ? 'Payment successful' : 'Checking your payment' }}</h1>
      <p class="text-sm text-gray-500 mt-2">
        {{ isPaid ? 'Your PromptPay payment has been confirmed.' : 'Stripe is confirming the payment. This page updates automatically.' }}
      </p>
...
async function checkStatus() {
  ...
  try {
    const { data } = await api.get(`/payments/stripe/status/${encodeURIComponent(orderId)}`)
    ...
    else errorMessage.value = 'Confirmation is taking longer than expected. You can check again from Orders.'
  } catch (error) {
    attempts += 1
    if (attempts < 5) timer = window.setTimeout(checkStatus, 2000)
    else errorMessage.value = error.response?.data?.error || 'Unable to check payment status.'
  }
}
