<template>
  <div class="min-h-screen bg-stone-50 pb-24">
    ...
    <main class="px-4 py-4 space-y-4">
      ...
      <div class="bg-white rounded-xl p-4 shadow-sm">
        ...
        <div class="space-y-3">
          ...
          >
            <!-- Product Image -->
            <div
              class="w-16 h-16 bg-stone-100 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden"
            >
              <img
                v-if="item.imageUrl"
                :src="item.imageUrl"
      ...
      <!-- Order Summary -->
      <div class="bg-white rounded-xl p-4 shadow-sm">
        <span class="text-base font-semibold text-gray-800 mb-4"
          >Order Summary</span
        >
        <div class="space-y-2 text-sm">
          ...
          <div
            class="border-t border-gray-100 pt-2 mt-2 flex justify-between font-semibold"
          >
      ...
      >
        ...
        <span>{{
          isLoading ? "Processing..." : "Place Order & Pay with PromptPay"
        }}</span>
      </button>
    </main>
...
async function placeOrder() {
  ...
  try {
    ...
    const orderTokensArray = savedOrderTokens
      ? JSON.parse(savedOrderTokens)
      : [];

    const finalOrderToken =
      orderResponse.data.order?.order_token || orderToken.value;
