...
test("Stripe checkout uses catalog prices and signed webhook controls payment status", async () => {
  ...
  await db
    .collection("products")
    .doc("product-one")
    .set({ name: "Towel", price: 125, weight: 0.6, is_active: true });
  await db
    .collection("orders")
    .doc("order-one")
    .set({
  ...
  async function request(
    path,
    { method = "GET", user, body, status = 200 } = {},
  ) {
  ...
  try {
    await request("/payments/stripe/checkout-session", {
      method: "POST",
      user: "other-user",
      body: { orderId: "order-one" },
      status: 403,
    });
    const checkout = await request("/payments/stripe/checkout-session", {
      method: "POST",
      user: "user-one",
      body: { orderId: "order-one", returnPath: "/orders?filter=Processing" },
    });
    ...
    assert.ok(
      createArgs.success_url.endsWith(
        "returnTo=%2Forders%3Ffilter%3DProcessing",
      ),
    );
    assert.ok(
      createArgs.cancel_url.endsWith(
        "returnTo=%2Forders%3Ffilter%3DProcessing",
      ),
    );
    ...
    await request("/payments/stripe/checkout-session", {
      method: "POST",
      user: "user-one",
      body: { orderId: "order-one", returnPath: "https://evil.example/steal" },
    });
    assert.ok(createArgs.success_url.endsWith("returnTo=%2Forders"));
    ...
    const payment = (
      await db.collection("payments").doc("order-one").get()
    ).data();
    assert.equal(payment.amount, 27000);
    assert.equal(
      (await db.collection("orders").doc("order-one").get()).data().total_price,
      270,
    );
    ...
    assert.equal(
      (await db.collection("orders").doc("order-one").get()).data()
        .payment_status,
      "paid",
    );
    assert.equal(
      (await request("/payments/stripe/status/order-one", { user: "user-one" }))
        .paymentStatus,
      "paid",
    );

    const duplicate = await request("/payments/stripe/webhook", {
      method: "POST",
      body: {},
    });
    ...
    await request("/payments/stripe/webhook", {
      method: "POST",
      body: {},
      status: 400,
    });
