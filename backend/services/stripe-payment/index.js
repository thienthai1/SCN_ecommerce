...
function safeReturnPath(value) {
  if (typeof value !== "string" || value.length > 2048) return "/orders";
  if (
...
function safeFailureReason(paymentIntent) {
  ...
  return error
    ? error.decline_code || error.code || "payment_failed"
    : "payment_failed";
}
...
async function resolveProduct(db, productId) {
  ...
  if (snapshot.exists)
    return { id: snapshot.id, data: snapshot.data(), suffix: "" };
...
async function resolveCatalogItem(db, item) {
  ...
  if (!Number.isInteger(quantity) || quantity <= 0)
    throw new Error("Order contains an invalid quantity");

  const product = await resolveProduct(db, item.product_id);
  if (!product || product.data.is_active === false)
    throw new Error("Order contains an unavailable product");
  ...
  if (product.data.isVariant === true || item.options || product.suffix) {
    const variants = await db
      ...
      .get();
    const requestedSku =
      item.options && item.options.sku
        ? String(item.options.sku)
        : product.suffix;
    const requestedName =
      item.options && item.options.name ? String(item.options.name) : "";
    const requestedType =
      item.options && item.options.variant ? String(item.options.variant) : "";
    let selected;
    ...
    for (const variantDoc of variants.docs) {
      ...
      selected = (variant.options || []).find(
        (option) =>
          (requestedSku && option.sku === requestedSku) ||
          (!requestedSku && requestedName && option.name === requestedName),
      );
      if (selected) break;
    }

    if (!selected || selected.isActive === false)
      throw new Error("Order contains an unavailable product option");
...
async function calculateOrder(db, orderId) {
  ...
  for (const itemDoc of itemSnapshot.docs)
    items.push(await resolveCatalogItem(db, itemDoc.data()));
  ...
  if (!shipping || !Number.isFinite(Number(shipping.pricePerKg)))
    throw new Error("Order has an invalid shipping type");
  ...
  const shippingAmount = toSatang(
    Number(shipping.pricePerKg) * chargeableWeight,
  );
...
module.exports = function registerStripePaymentRoutes(app, opts = {}) {
  ...
  async function ownedOrder(req, res) {
    const orderId =
      req.body && req.body.orderId ? req.body.orderId : req.params.orderId;
    if (!orderId) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!owned) return;
        if (
          owned.data.payment_status === "paid" ||
          owned.data.payment_status === "refunded"
        ) {
        ...
        if (calculated.shippingAmount > 0) {
          lineItems.push({
            price_data: {
              ...
              unit_amount: calculated.shippingAmount,
            },
            quantity: 1,
          });
        }
        ...
        const session = await stripe.checkout.sessions.create(
          {
            ...
            success_url: `${baseUrl}/payment/success?orderId=${encodeURIComponent(owned.id)}&returnTo=${returnQuery}`,
            cancel_url: `${baseUrl}/payment/cancel?orderId=${encodeURIComponent(owned.id)}&returnTo=${returnQuery}`,
          },
  ...
  app.get(
    "/payments/stripe/status/:orderId",
    authenticate,
    async (req, res) => {
      try {
        ...
        return res.json({
          orderId: owned.id,
          paymentStatus: owned.data.payment_status || "unpaid",
        });
      } catch (error) {
  ...
  app.post("/payments/stripe/webhook", async (req, res) => {
    ...
    try {
      ...
      if (
        (!paymentSnapshot || !paymentSnapshot.exists) &&
      ...
      if (
        ...
        if (!orderId || !paymentSnapshot || !paymentSnapshot.exists)
        ...
        const updates = {
          ...
          processed_event_ids: FieldValue.arrayUnion(event.id),
        };
        const orderUpdates = {};
        ...
        if (event.type === "payment_intent.succeeded") {
          if (
            ...
            object.amount_received !== payment.amount
          )
          ...
          if (
            object.currency !== payment.currency ||
            object.amount !== payment.amount ||
            object.amount_refunded > payment.amount
          ) {
            throw new Error("Stripe amount or currency mismatch");
          }
          const refunded =
      ...
      await webhookEvents
        .doc(event.id)
        .set({ type: event.type, processed_at: FieldValue.serverTimestamp() });
